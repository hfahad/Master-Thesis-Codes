...
def train(data_dir,
    ...
    atlas_vol =sitk.GetArrayFromImage(sitk.ReadImage(atlas_file))[np.newaxis, ..., np.newaxis]
    atlas_vol=(atlas_vol-atlas_vol.min())/(atlas_vol.max()-atlas_vol.min())
    ...
    #train_vol_names = glob.glob(os.path.join(data_dir, '*.gz'))
    #f,m= datagenerators.load_volume_liver(data_dir)
    multi= datagenerators.load_volume_liver(data_dir)
        
#random.shuffle(train_vol_names)  # shuffle volume list
    assert len(m) > 0, "Could not find any training data"
    #assert len(f) > 0, "Could not find any training data"
    ...
    assert np.mod(batch_size, nb_gpus) == 0, \
     ...
     #------------------------------------- For multi model-------------------------------
    train_example_gen = datagenerators.example_gen(multi,batch_size=batch_size)
    ...
    #------------------------- For Random selection of fixed volume in each epochs--------------
  #  train_example_gen = datagenerators.example_gen(multi,batch_size=batch_size)
  #  train_example_gen1 = datagenerators.example_gen(mono,batch_size=batch_size)
    
   # miccai2018_gen = datagenerators.miccai2018_multimodel(train_example_gen,
    #                                               train_example_gen1,
     #                                              batch_size=batch_size,
      #                                             bidir=bidir)
    ...
    with tf.device(gpu):
        ...
        # single gpu
            ...
            try:
                ...
                csv_logger=tf.keras.callbacks.CSVLogger('../Models/multimodel_200_0.1/training.log')
            except:
                pass
            mg_model = model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--atlas_file", type=str,
                        dest="atlas_file", default='data/700005_HELENA_20150527_11_3D_GRE_TRA_HELENA_W.nrrd',
                        help="gpu id number")
    parser.add_argument("--model_dir", type=str,
                        dest="model_dir", default='../Models/multimodel_200_0.1',
                        help="models folder")
    ...
    parser.add_argument("--epochs", type=int,
                        dest="nb_epochs", default=2000,
                        help="number of iterations")
    parser.add_argument("--prior_lambda", type=float,
                        dest="prior_lambda", default=200,
