...
def get_do_separate_z(old_spacing, new_spacing):
    do_separate_z = (np.max(new_spacing) / np.min(new_spacing)) > RESAMPLING_SEPARATE_Z_ANISOTROPY_THRESHOLD or \
                    (np.max(old_spacing) / np.min(old_spacing)) > RESAMPLING_SEPARATE_Z_ANISOTROPY_THRESHOLD
...
def resample_patient(data, seg, original_spacing, target_spacing, order_data=3, order_seg=0, force_separate_z=False,
    ...
    new_shape = np.array(((np.array(original_spacing) / np.array(target_spacing)).astype(float) * shape)).astype(int)

    if force_separate_z is not None:
        do_separate_z = force_separate_z
    else:
        do_separate_z = get_do_separate_z(original_spacing, target_spacing)
