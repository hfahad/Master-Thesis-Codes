...
class Miccai2018():
    ...
    def _MINDSSC3D(self, data,vol_shape):
        if isinstance(vol_shape.shape, (tf.Dimension, tf.TensorShape)):
            volshape = vol_shape.shape.as_list()
        else:
            volshape = vol_shape.shape

        H = volshape[1];
        W = volshape[2];
        D = volshape[3]

        vol = tf.reshape(data, (H, W, D,1))
    ...
    # define spatial offset layout for 12 self-similarity patches
        ...
        theta_ssc[0, :, 0] = np.array([ 0,  0, -h, -h,  0,  0,  0,  0, +h, +h,  0,  0])
        theta_ssc[0, :, 1] = np.array([ 0,  0,  0,  0, +d, +d, -d, -d,  0,  0,  0,  0])
        theta_ssc[0, :, 2] = np.array([-w, -w,  0,  0,  0,  0,  0,  0,  0,  0, +w, +w])
        theta_ssc[1, :, 0] = np.array([-h,  0,  0,  0, -h, +h,  0, +h,  0,  0,  0,  0])
        theta_ssc[1, :, 1] = np.array([ 0, +d, -d,  0,  0,  0,  0,  0,  0,  0, -d, +d])
        theta_ssc[1, :, 2] = np.array([ 0,  0,  0, +w,  0,  0, -w,  0, -w, +w,  0,  0])

    # computing the shifting by filter
        sampledd = []
        for i in range(12):
            shift = np.zeros((H, W, D, 3))
        ...
        my_mind = tf.keras.layers.AveragePooling3D(pool_size=(kernel, kernel, kernel), strides=(1, 1, 1), padding='same',
                                               data_format='channels_last')(tf.abs(sampledd) ** 2)
    ...
    def recon_loss(self, y_true, y_pred):
        ...
        y_pred_m= self._MINDSSC3D(y_pred,y_pred)
        y_true_m= self._MINDSSC3D(y_true,y_pred)

        return 1. / (self.image_sigma**2) * tf.reduce_sum(K.square(y_true_m[0,...] - y_pred_m[0,...]))
